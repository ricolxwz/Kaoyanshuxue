...
设$ f(x) $和$ g(x) $在$ x=0 $的某邻域内连续, 且$ \lim\limits_{x \rightarrow 0}\frac{f(x)}{g(x)}=1 $, 则:
\begin{equation*}
\displaystyle\int_{0}^{x}\hspace{.2em}f(t)\mathrm{d}t\sim \displaystyle\int_{0}^{x}\hspace{.2em}g(t)\mathrm{d}t
\end{equation*}
\subsection{无穷大}
...
\begin{equation*}
y^{*}=x_{k}e^{\alpha x}[R_{m}^{(1)}(x)\cos \beta x+R_{m}^{(2)}(x)\sin \beta x], m=\mathrm{max}\{l,n\}
\end{equation*}\par $ k $的取值如下:
\begin{enumerate}
